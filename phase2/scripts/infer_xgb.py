"""Inference for the hierarchical XGBoost bundle (train_xgb_hier_aws.py).

Mirrors infer_hierarchical.py / infer_scanvi.py but uses the HierXGBBundle's
.predict() to get all 4 levels in one call.
"""
...
# Make HierXGBBundle importable so joblib unpickle works (saved by
# train_xgb_hier_aws as __main__.HierXGBBundle).
sys.path.insert(0, str(Path(__file__).parent))
from train_xgb_hier_aws import HierXGBBundle  # noqa: E402, F401
import __main__
__main__.HierXGBBundle = HierXGBBundle
...
def main(argv=None) -> int:
    ...
    p.add_argument("--bundle", required=True, help="xgb_bundle.joblib path")
    ...
    preproc = bundle_obj.get("preproc", "log1p")

    if bundle_obj.get("type") == "hier_xgb_modal":
        # Modal-style bundle: dict of {models, encoders, levels} — no wrapper class
        models = bundle_obj["models"]
        encoders = bundle_obj["encoders"]
        levels_internal = tuple(bundle_obj["levels"])
        feature_dim = bundle_obj.get("feature_dim", "?")
        ...
        class _ModalBundle:
            def predict(self, X):
                ...
                for lvl in levels_internal:
                    ...
                    pred = models[lvl].predict(feats)
                    out[lvl] = encoders[lvl].inverse_transform(pred)
                    prev = models[lvl].predict_proba(feats)
                return out
        bundle = _ModalBundle()
        print(f"  Modal XGB bundle; preproc={preproc} feature_dim={feature_dim}")
    else:
        bundle = bundle_obj["bundle"]  # HierXGBBundle (local-trained variant)
        print(f"  XGB bundle loaded; preproc={preproc} feature_dim={bundle.feature_dim}")
    ...
    for fov in fov_list:
        ...
        if len(cell_ids) == 0:
            ...
            Xq = _preprocess(X, preproc)
            cell_predictions = bundle.predict(Xq)
