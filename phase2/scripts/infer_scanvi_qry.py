"""scANVI inference using prepare_query_anndata + load_query_data pattern.

Replacement for infer_scanvi.py that works with scvi-tools 1.4+ where
SCANVI.load(adata=None) is no longer supported. Uses the official query-mode
inference flow:
    1. Build AnnData from per-cell counts (with placeholder fov + label cols)
    2. SCANVI.prepare_query_anndata(qry, ref_dir) — aligns gene order, etc.
    3. SCANVI.load_query_data(qry, ref_dir) — instantiates model on qry
    4. model.predict(qry) → labels

Bundle layout expected (from modal_train_scanvi_xl.py):
    <bundle_dir>/scanvi_bundle.joblib
    <bundle_dir>/scanvi_per_level/{class_label,subclass_label,...}/  ← model dirs
"""
...
def main(argv=None) -> int:
    ...
    if candidate_local.is_dir():
        ...
        raise FileNotFoundError(f"missing scanvi_per_level/ next to {args.bundle}")
    ...
    # Load all 4 scANVI models once (reusing across FOVs)
    print("loading 4 scANVI models...")
    scanvi_paths = {lvl: str(manifest_dir / lvl) for lvl in LEVELS_INTERNAL}
        ...
        if len(cell_ids) == 0:
            ...
            for lvl in LEVELS_INTERNAL:
                qry = ad.AnnData(
                    X=X.astype(np.int32),
                    obs=pd.DataFrame({
                        "fov": [fov] * len(cell_ids),
                        lvl: ["UNLABELED"] * len(cell_ids),
                    }, index=[str(c) for c in cell_ids]),
                    var=pd.DataFrame(index=genes),
                )
                # Align query to ref's gene/category space, then load model on it
                scvi.model.SCANVI.prepare_query_anndata(qry, scanvi_paths[lvl])
                m = scvi.model.SCANVI.load_query_data(qry, scanvi_paths[lvl])
                pred = np.asarray(m.predict(qry)).astype(str)
                cell_predictions[lvl] = pred
